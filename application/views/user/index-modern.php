<div class="page-header">
    <div class="page-header-content">
        <div>
            ...
            <p class="page-subtitle">Kelola akun pengguna dan pegawai</p>
        </div>
        <div class="header-actions">
            <a href="<?= site_url('user/create') ?>" class="btn btn-outline">
                <i class="fas fa-plus"></i>
                <span>Tambah Manual</span>
            </a>
            <a href="<?= site_url('user/import') ?>" class="btn btn-primary">
    ...
    <div class="card">
        <div class="data-table-header">
            <div class="data-table-search">
                ...
                <input type="text" id="searchInput" placeholder="Cari pengguna..." value="<?= htmlspecialchars($search) ?>">
                <?php if (!empty($search)): ?>
                    ...
                    <tr>
                        <td colspan="8" class="text-center">
                            <div class="table-empty">
                                ...
                                <p><?= !empty($search) ? 'Tidak ada hasil pencarian' : 'Belum ada pengguna' ?></p>
                            </div>
                        </td>
                    </tr>
                    <?php else: ?>
                        ...
                        <td>
                            ...
                            <div class="action-buttons">
                                ...
                                <button class="btn btn-sm btn-outline warning" 
                                        onclick="confirmAction('<?= site_url('user/deactivate/' . $user['id_user']) ?>', 'Nonaktifkan pengguna <?= htmlspecialchars($user['nama'] ?? $user['username']) ?>?')"
                                        title="Nonaktifkan">
                                    <i class="fas fa-user-slash"></i>
                                </button>
                                <?php else: ?>
                                <button class="btn btn-sm btn-outline success" 
                                        onclick="confirmAction('<?= site_url('user/activate/' . $user['id_user']) ?>', 'Aktifkan pengguna <?= htmlspecialchars($user['nama'] ?? $user['username']) ?>?')"
                                        title="Aktifkan">
...
<div class="custom-modal" id="actionModal">
    ...
    <div class="custom-modal-content">
        ...
        <div class="custom-modal-footer">
            ...
            <a href="#" id="actionLink" class="btn btn-primary">Ya, Lanjutkan</a>
        </div>
    </div>
</div>
...
function confirmAction(url, message) {
    ...
    jQuery('#actionModal').fadeIn(200);
    jQuery('body').css('overflow', 'hidden');
}

function closeModal() {
    jQuery('#actionModal').fadeOut(200);
...
.user-avatar { width: 36px; height: 36px; border-radius: 50%; background: linear-gradient(135deg, #3b82f6, #2563eb); color: white; display: flex; align-items: center; justify-content: center; font-weight: 600; font-size: 14px; }
...
.btn-outline { padding: 6px 10px; border: 1px solid #e5e7eb; border-radius: 6px; background: white; cursor: pointer; transition: all 0.15s; }
.btn-outline.warning { color: #f59e0b; }
.btn-outline.warning:hover { background: #f59e0b; color: white; }
.btn-outline.success { color: #10b981; }
